...
Subscription::Subscription(const std::string &name, const std::string& md5sum, const std::string& datatype, const ros::TransportHints& transport_hints, ros::PollManagerPtr& poll_manager )
: name_(name)
, md5sum_(md5sum)
, datatype_(datatype)
...
void Subscription::addLocalConnection(const ros::PublicationPtr& pub)
{
  ...
  ROS_INFO("Creating intraprocess link for topic [%s]", name_.c_str());
/*
  ros::IntraProcessPublisherLinkPtr pub_link(new ros::IntraProcessPublisherLink(shared_from_this(), XMLRPCManager::instance()->getServerURI(), transport_hints_));
  ros::IntraProcessSubscriberLinkPtr sub_link(new ros::IntraProcessSubscriberLink(pub));
...
bool urisEqual(const std::string& uri1, const std::string& uri2)
{
  ...
  ros::network::splitURI(uri1, host1, port1);
  ros::network::splitURI(uri2, host2, port2);
  return port1 == port2 && host1 == host2;
}

bool Subscription::pubUpdate(const ros::V_string& new_pubs)
{
...

  {
    ...
    for (ros::V_string::const_iterator up_i = new_pubs.begin();
  ...
  // it doesn't really matter.
  {
    ...
    for (ros::V_string::const_iterator up_i  = new_pubs.begin(); up_i != new_pubs.end(); ++up_i)
    {
      bool found = false;
      for (V_PublisherLink::iterator spc = publisher_links_.begin();
  ...
  for (ros::V_string::iterator i = additions.begin();
            i != additions.end(); ++i)
  {
...
//ROS_INFO("WE RECEIVED A MESSAGE!!");
...
bool Subscription::addCallback(const SubscriptionCallbackHelperPtr& helper, const std::string& md5sum, ros::CallbackQueueInterface* queue, int32_t queue_size, const ros::VoidConstPtr& tracked_object, bool allow_concurrent_callbacks)
{
  ROS_ASSERT(helper);
  ROS_ASSERT(queue);
...

  {
    ...
    info->subscription_queue_.reset(new ros::SubscriptionQueue(name_, queue_size, allow_concurrent_callbacks));
      ...
      for (; it != end;++it)
      {
        ...
        if (link->isLatched())
        {
          ...
          if (des_it != latched_messages_.end())
          {
            ...
            ros::MessageDeserializerPtr des(new ros::MessageDeserializer(helper, latch_info.message, latch_info.connection_header));
            bool was_full = false;
