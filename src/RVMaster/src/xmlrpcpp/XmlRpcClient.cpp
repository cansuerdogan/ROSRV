...
{
  XmlRpc::XmlRpcUtil::log(1, "XmlRpcClient new client: host %s, port %d.", host, port);
...
XmlRpcClient::close()
{
  XmlRpc::XmlRpcUtil::log(4, "XmlRpcClient::close: fd %d.", getfd());
  _connectionState = NO_CONNECTION;
  _disp.exit();
...
// might be a fault).
bool
XmlRpcClient::execute(const char* method, XmlRpc::XmlRpcValue const& params, XmlRpc::XmlRpcValue& result)
{
...
// might be a fault).
bool
XmlRpcClient::executeNonBlock(const char* method, XmlRpc::XmlRpcValue const& params)
{
  XmlRpc::XmlRpcUtil::log(1, "XmlRpcClient::execute: method %s (_connectionState %d).", method, _connectionState);
...
bool
XmlRpcClient::executeCheckDone(XmlRpc::XmlRpcValue& result)
{
  result.clear();
  // Are we done yet?
...
XmlRpcClient::handleEvent(unsigned eventType)
{
  if (eventType == XmlRpcDispatch::Exception)
  {
    if (_connectionState == WRITE_REQUEST && _bytesWritten == 0)
      XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::handleEvent: could not connect to server (%s).", 
                       XmlRpcSocket::getErrorMsg().c_str());
    else
      XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::handleEvent (state %d): %s.", 
...
XmlRpcClient::doConnect()
{
  ...
  if (fd < 0)
  {
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::doConnect: Could not create socket (%s).", XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }

  XmlRpc::XmlRpcUtil::log(3, "XmlRpcClient::doConnect: fd %d.", fd);
  this->setfd(fd);
  ...
  {
    this->close();
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::doConnect: Could not set socket to non-blocking IO mode (%s).", XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
  ...
  {
    this->close();
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::doConnect: Could not connect to server (%s).", XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
...
bool
XmlRpcClient::generateRequest(const char* methodName, XmlRpc::XmlRpcValue const& params)
{
  ...
  XmlRpc::XmlRpcUtil::log(4, "XmlRpcClient::generateRequest: header is %d bytes, content-length is %d.", 
...
{
  if (_bytesWritten == 0)
    XmlRpc::XmlRpcUtil::log(5, "XmlRpcClient::writeRequest (attempt %d):\n%s\n", _sendAttempts+1, _request.c_str());

  // Try to write the request
  if ( ! XmlRpcSocket::nbWrite(this->getfd(), _request, &_bytesWritten)) {
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::writeRequest: write error (%s).",XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
    
  XmlRpc::XmlRpcUtil::log(3, "XmlRpcClient::writeRequest: wrote %d of %d bytes.", _bytesWritten, _request.length());
...
XmlRpcClient::readHeader()
{
  ...
  if ( ! XmlRpcSocket::nbRead(this->getfd(), _header, &_eof) ||
    ...
    if (getKeepOpen() && _header.length() == 0 && _sendAttempts++ == 0) {
      XmlRpc::XmlRpcUtil::log(4, "XmlRpcClient::readHeader: re-trying connection");
      XmlRpcSource::close();
      _connectionState = NO_CONNECTION;
      _eof = false;
    ...
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::readHeader: error while reading header (%s) on fd %d.",
                      XmlRpcSocket::getErrorMsg().c_str(), getfd());
    return false;
  }

  XmlRpc::XmlRpcUtil::log(4, "XmlRpcClient::readHeader: client has read %d bytes", _header.length());
  ...
  if (bp == 0) {
    ...
    {
      XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::readHeader: EOF while reading header");
  ...
  if (lp == 0) {
    XmlRpc::XmlRpcUtil::error("Error XmlRpcClient::readHeader: No Content-length specified");
    return false;   // We could try to figure it out by parsing as we read, but for now...
  }

  _contentLength = atoi(lp);
  if (_contentLength <= 0) {
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::readHeader: Invalid Content-length specified (%d).", _contentLength);
    return false;
  }
  	
  XmlRpc::XmlRpcUtil::log(4, "client read content length: %d", _contentLength);
...
XmlRpcClient::readResponse()
{
  ...
  if (int(_response.length()) < _contentLength) {
    if ( ! XmlRpcSocket::nbRead(this->getfd(), _response, &_eof)) {
      XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::readResponse: read error (%s).",XmlRpcSocket::getErrorMsg().c_str());
      return false;
    }
    ...
    if (int(_response.length()) < _contentLength) {
      if (_eof) {
        XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::readResponse: EOF while reading response");
        return false;
      }
      return true;
    }
  ...
  XmlRpc::XmlRpcUtil::log(3, "XmlRpcClient::readResponse (read %d bytes)", _response.length());
  XmlRpc::XmlRpcUtil::log(5, "response:\n%s", _response.c_str());
...
bool
XmlRpcClient::parseResponse(XmlRpc::XmlRpcValue& result)
{
  // Parse response xml into result
  int offset = 0;
  if ( ! XmlRpc::XmlRpcUtil::findTag(METHODRESPONSE_TAG,_response,&offset)) {
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid response - no methodResponse. Response:\n%s", _response.c_str());
    return false;
  }

  // Expect either <params><param>... or <fault>...
  if ((XmlRpc::XmlRpcUtil::nextTagIs(PARAMS_TAG,_response,&offset) &&
       XmlRpc::XmlRpcUtil::nextTagIs(PARAM_TAG,_response,&offset)) ||
      (XmlRpc::XmlRpcUtil::nextTagIs(FAULT_TAG,_response,&offset) && (_isFault = true)))
  {
    if ( ! result.fromXml(_response, &offset)) {
      XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid response value. Response:\n%s", _response.c_str());
      _response = "";
      return false;
    }
  } else {
    XmlRpc::XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid response - no param or fault tag. Response:\n%s", _response.c_str());
    _response = "";
    return false;
  }
