...
{
  XmlRpc::XmlRpcUtil::log(4, "XmlRpcSocket::close: fd %d.", fd);
#if defined(_WINDOWS)
  closesocket(fd);
#else
  ::close(fd);
...
int
XmlRpcSocket::accept(int fd,rv::ClientInfo &ci)
{
...
printf("user request from ip address %s port %d \n",ip,inaddr_ptr->sin_port);

ci.family=inaddr_ptr->sin_family;
ci.port = inaddr_ptr->sin_port;
ci.ip=std::string(ip);
...
XmlRpcSocket::connect(int fd, std::string& host, int port)
{
  ...
  for (; it; it = it->ai_next)
  {
    if (!s_use_ipv6_ && it->ai_family == AF_INET)
    {
      ...
      XmlRpc::XmlRpcUtil::log(5, "found host as %s\n", inet_ntoa(address->sin_addr));
      found = true;
      break;
    }
...
#endif
    XmlRpc::XmlRpcUtil::log(5, "XmlRpcSocket::nbRead: read/recv returned %d.", n);
...
#endif
    XmlRpc::XmlRpcUtil::log(5, "XmlRpcSocket::nbWrite: send/write returned %d.", n);
